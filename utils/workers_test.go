...
// BenchmarkRegisterTaskWithDelay measures the performance of task registration with delay.
func BenchmarkRegisterTaskWithDelay(b *testing.B) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		RegisterWorkerOnDelay(ctx, callback, 1*time.Second)
	}
}

// BenchmarkRegisterTaskOnSignal measures the performance of task registration with signals.
func BenchmarkRegisterTaskOnSignal(b *testing.B) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	signal := make(chan any, b.N)
	callback := func() {}
...
// BenchmarkCallWithRetries measures the performance of retry logic.
func BenchmarkCallWithRetries(b *testing.B) {
	callback := func() error {
		return nil
	}
